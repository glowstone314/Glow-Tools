...
public class EllipsoidHandler implements ShapeStrategy {
    ...
    private BlockPos[] calcMinMax(BlockPos origin) {
        ...
        BlockPos min = origin.offset(-w / 2, -h / 2, -d / 2);
        BlockPos max = min.offset(w - 1, h - 1, d - 1);
    ...
    public int buildUI(GuiBase gui, int x, int y, int labelWidth, int fieldWidth, int gap, List<Object> registry) {
        ...
        tfW.setValue(String.valueOf(width)); tfH.setValue(String.valueOf(height)); tfD.setValue(String.valueOf(depth));

        gui.addTextField(tfW, f -> { try { width = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
        gui.addTextField(tfH, f -> { try { height = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
        gui.addTextField(tfD, f -> { try { depth = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
