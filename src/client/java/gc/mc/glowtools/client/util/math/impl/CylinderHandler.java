...
public class CylinderHandler implements ShapeStrategy {
    ...
    private BlockPos[] calcMinMax(BlockPos origin) {
        ...
        BlockPos min = origin.offset(-dims[0] / 2, -dims[1] / 2, -dims[2] / 2);
        BlockPos max = min.offset(dims[0] - 1, dims[1] - 1, dims[2] - 1);
                ...
                for (int z = 0; z < d; z++) {
                    if (inside[x][y][z]) {
                        if (hollow) {
                            ...
                            if (isBoundary) positions.add(minCorner.offset(x, y, z));
                        } else {
                            positions.add(minCorner.offset(x, y, z));
                        }
                    }
                }
            }
    ...
    public int buildUI(GuiBase gui, int x, int y, int labelWidth, int fieldWidth, int gap, List<Object> registry) {
        ...
        tfLen.setValue(String.valueOf(cylinderLength));
        gui.addTextField(tfLen, f -> { try { cylinderLength = Integer.parseInt(f.getValue().trim()); }
        ...
        GuiTextFieldInteger tfElLength = new GuiTextFieldInteger(xOff, y, fieldWidth, 20, gui.font);
        GuiTextFieldInteger tfElWidth = new GuiTextFieldInteger(xOff + fieldWidth + gap, y, fieldWidth, 20, gui.font);

        tfElLength.setValue(String.valueOf(ellipseLength));
        tfElWidth.setValue(String.valueOf(ellipseWidth));

        gui.addTextField(tfElLength, f -> { try { ellipseLength = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
        gui.addTextField(tfElWidth, f -> { try { ellipseWidth = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
