...
public class DiagonalHandler implements ShapeStrategy {
    ...
    public List<BlockPos> calculate(BlockPos origin) {
        ...
        BlockPos start = origin.offset(startX, startY, startZ);
        BlockPos end = origin.offset(endX, endY, endZ);
    ...
    public int buildUI(GuiBase gui, int x, int y, int labelWidth, int fieldWidth, int gap, List<Object> registry) {
        ...
        GuiTextFieldInteger tfSx = new GuiTextFieldInteger(xOff, y, fieldWidth, 20, gui.font);
        GuiTextFieldInteger tfSy = new GuiTextFieldInteger(xOff + fieldWidth + gap, y, fieldWidth, 20, gui.font);
        GuiTextFieldInteger tfSz = new GuiTextFieldInteger(xOff + 2 * (fieldWidth + gap), y, fieldWidth, 20, gui.font);

        tfSx.setValue(String.valueOf(startX)); tfSy.setValue(String.valueOf(startY)); tfSz.setValue(String.valueOf(startZ));

        gui.addTextField(tfSx, f -> { try { startX = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
        gui.addTextField(tfSy, f -> { try { startY = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
        gui.addTextField(tfSz, f -> { try { startZ = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
        ...
        GuiTextFieldInteger tfEx = new GuiTextFieldInteger(xOff, y, fieldWidth, 20, gui.font);
        GuiTextFieldInteger tfEy = new GuiTextFieldInteger(xOff + fieldWidth + gap, y, fieldWidth, 20, gui.font);
        GuiTextFieldInteger tfEz = new GuiTextFieldInteger(xOff + 2 * (fieldWidth + gap), y, fieldWidth, 20, gui.font);

        tfEx.setValue(String.valueOf(endX)); tfEy.setValue(String.valueOf(endY)); tfEz.setValue(String.valueOf(endZ));

        gui.addTextField(tfEx, f -> { try { endX = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
        gui.addTextField(tfEy, f -> { try { endY = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
        gui.addTextField(tfEz, f -> { try { endZ = Integer.parseInt(f.getValue().trim()); }
        catch (Exception ignored) {} return false; }, TextFieldType.INTEGER);
